...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = ` <main class="${["palette-plain", activeMouse ? "activeMouse" : ""].join(" ").trim()}"><input id="uploadId" type="file" hidden> <div class="${[
    ...
    ].join(" ").trim()}"><div class="sidebar svelte-zlzldx"><div class="header svelte-zlzldx">${validate_component(ButtonBar, "ButtonBar").$$render(
      $$result,
      {
        resize: true,
        buttons: [
      ...
      {},
      {}
    )} ${$isLoggedIn ? `<div class="cloud-status svelte-zlzldx"><span class="${[
      "save-indicator svelte-zlzldx",
      ...
      "tabs svelte-zlzldx",
      settings.data.customScrollbar.value ? "customScrollbar" : ""
    ].join(" ").trim()}"><div class="tabScroll svelte-zlzldx">${$isLoggedIn && $cloudFlowList.length > 0 ? `<div class="cloud-flows-section svelte-zlzldx"><div class="section-label svelte-zlzldx" data-svelte-h="svelte-1g1s9en">Your Flows</div> ${each($cloudFlowList, (flow) => {
      return `<div class="cloud-flow-item-wrapper svelte-zlzldx">${editingFlowId === flow.id ? `<input id="cloud-flow-edit-input" class="cloud-flow-edit-input svelte-zlzldx"${add_attribute("value", editingFlowTitle, 0)}>` : `<button class="${[
        "cloud-flow-item svelte-zlzldx",
        ($currentCloudFlowId === flow.id ? "active" : "") + " " + (flow.is_shared ? "shared" : "")
      ].join(" ").trim()}"><span class="flow-title svelte-zlzldx">${escape(flow.title || "Untitled")}</span> ${flow.is_shared ? `<span class="shared-badge svelte-zlzldx" data-svelte-h="svelte-ox5vk7">shared</span>` : ``} </button>`} <button class="cloud-flow-delete svelte-zlzldx" title="Delete flow" data-svelte-h="svelte-1spinoe">×</button> </div>`;
    })}</div> <button class="new-flow-btn svelte-zlzldx" data-svelte-h="svelte-1j1xl8p">+ New Flow</button> <div class="section-divider svelte-zlzldx"></div> <div class="section-label svelte-zlzldx" data-svelte-h="svelte-15dxj9u">Current Flow Tabs</div>` : ``} ${validate_component(SortableList, "SortableList").$$render($$result, { list: $nodes.root.children }, {}, {
      default: ({ index }) => {
      ...
      {}
    )}</div></div> <div class="sidebar-footer svelte-zlzldx">${$isLoggedIn ? `<button class="sidebar-action-btn share-btn svelte-zlzldx" ${!$currentCloudFlowId ? "disabled" : ""}>Share Flow</button> <button class="sidebar-action-btn signout-btn svelte-zlzldx" data-svelte-h="svelte-16upgnz">Sign Out</button>` : ``}</div> <div class="timer">${validate_component(Timers, "Timers").$$render($$result, {}, {}, {})}</div></div> ${$authLoading ? `<div class="prelude svelte-zlzldx" data-svelte-h="svelte-1ei1ena"><div class="loading-message svelte-zlzldx">Loading...</div></div>` : `${!$isLoggedIn ? `<div class="prelude svelte-zlzldx">${validate_component(Auth, "Auth").$$render($$result, {}, {}, {})}</div>` : `${$nodes.root.children.length > 0 ? `${$selectedFlowId != null && $nodes[$selectedFlowId] ? `<div class="title svelte-zlzldx">${validate_component(Title, "Title").$$render(
      $$result,
      {
        flowId: $selectedFlowId,
        deleteSelf: () => deleteFlowAndFocus()
      ...
      {},
      {}
    )}</div> <div class="box-control svelte-zlzldx">${validate_component(BoxControl, "BoxControl").$$render($$result, { flowId: $selectedFlowId }, {}, {})}</div> <div class="${[
      "flow svelte-zlzldx",
      settings.data.customScrollbar.value ? "customScrollbar" : ""
    ].join(" ").trim()}">${validate_component(Flow, "Flow").$$render($$result, { flowId: $selectedFlowId }, {}, {})}</div>` : ``} ${showSideDoc ? `<div class="side-doc svelte-zlzldx">${validate_component(SideDoc, "SideDoc").$$render($$result, {}, {}, {})}</div>` : ``}` : `<div class="prelude svelte-zlzldx">${validate_component(Prelude, "Prelude").$$render($$result, {}, {}, {})}</div>`}`}`}</div> </main>`;
  } while (!$$settled);
  $$unsubscribe_selectedFlowId();
  $$unsubscribe_activeMouse();
  $$unsubscribe_nodes();
